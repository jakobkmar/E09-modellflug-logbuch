...
| Piloten | Nutzer des Flugplatzes | Sie müssen jeden Flug im Logbuch am Ende ihrer Session vollständig eintragen
| Mitarbeiter des Flugplatzes | verantwortliches Personal | sind Aufseher des Platzes und kontrollieren die Vollständigkeit der Logbücher; kontrollieren die Funktionsfähigkeit der Technik auf dem Flugplatz (Wetterstation, Datenbank, Login-Tool)
| Forschende | Professoren einer Universität | können anhand der gesammelten digitalen und somit strukturierten Daten Studien anstellen
| andere Flugplätze | Betreiber eines weiteren Flugplatzes | möchte auch ein digitales Logbuch implementieren oder nutzt es bereits und will beide Systeme koppeln, um Informationen live auszutauschen
|===
...
| xx     | xx        | Nutzer können sich mit Nutzernamen und Passwort anmelden               | xx
|        |           | Administrator kann Nutzer bearbeiten und Webapp pflegen und bearbeiten |
|        |           | Die Wichtigsten Daten des für das Logbuch können erfasst werden        |
|        |           | Unwichhtigere Daten können in erweiterten Optionen erfasst werden      |
|        |           | Sprache Deutsch                                                        |
|        |           | Sprache Englisch                                                       |
|        |           | Wetterdaten werden automazisch erfasst sobald Wetterstation online ist |
|        |           | _Platzhalter rechtlich einzuhaltende Bedingungen_                      |
