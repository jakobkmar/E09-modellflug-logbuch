...
Zur Überprüfung wird ein Testnutzer ein Protokoll erstellen. Es sollte innerhalb von 2 Minuten (?)erstellt und verarbeitet sein.
...
Zur Überprüfung wird eine Wartungssitzung simuliert und überprüft, dass der Zeitaufwand für diese im Rahmen bleibt. (2h pro Jahr?)
